...
motivation: 传统的生成模型在面对分布偏移的目标域时，往往需要复杂的重新训练或参数适配，限制了其在目标域数据增强中的应用。
method: 提出SGN框架，利用编码器-解码器结构学习受标签相似性约束的潜空间，并在生成阶段通过融合少量目标域代表性样本的潜编码来生成新数据。
result: 在图像和表格数据集上的实验表明，SGN能够有效生成具有目标域特征且保持类别一致性的样本，显著提升了分布偏移下的数据增强效果。
conclusion: SGN提供了一种无需微调即可应对分布偏移的通用生成方案，通过结构化潜空间实现了高效的目标导向数据生成。
---

## 摘要
在源域上训练的生成模型产生的样本往往与发生偏移的目标域对齐较差，从而限制了它们在目标域数据增强中的有效性。尽管针对特定目标的适配可以减少这种不匹配，但它通常需要额外的优化和领域特定的参数。我们提出了一种基于相似性的生成网络（SGN），这是一个可重用的框架，在有标签的源数据上训练一次后，即可应用于新的目标域而无需更新参数。SGN 通过编码器-解码器架构在保留重构信息的同时，学习一个由标签诱导的两两相似性构建的潜空间。在生成阶段，来自目标域的一小部分有标签代表性集合被编码并在学习到的潜空间中进行组合，使得生成的样本在保持类别一致性的同时继承目标特定的特征。我们进一步分析了所提出的相似性结构的可实现性和维度要求。在图像和表格数据集上的实验证明了 SGN 在源到目标的分布偏移下进行目标引导数据增强的有效性。
