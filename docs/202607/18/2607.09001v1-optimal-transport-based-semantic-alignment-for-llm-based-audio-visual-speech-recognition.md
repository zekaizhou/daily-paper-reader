...
title_zh: 基于最优传输的 LLM 视听语音识别语义对齐
authors: "Xugang Lu, Peng Shen, Yu Tsao, Hisashi Kawai"
date: 2026-07-10
pdf: "https://arxiv.org/pdf/2607.09001v1"
tags: ["query:vlm-da"]
score: 7.0
evidence: 基于LLM的视听语音识别的软提示
tldr: 针对基于大语言模型（LLM）的音视频语音识别（AVSR）中存在的模态表征差异问题，本文提出了一种基于最优传输（OT）的语义对齐框架。该方法在多模态融合前，利用OT将音频和视频特征与LLM的语言嵌入空间进行对齐，并通过对比学习增强语义一致性。实验表明，该框架在LRS3-TED基准测试中显著提升了鲁棒性，在多种噪声环境下均达到了SOTA性能。
source: arxiv
...
motivation: 现有的LLM-AVSR方法在融合多模态特征时，未显式解决音频、视频与文本模态间的表征差异，限制了跨模态集成的效果。
method: 提出利用最优传输（OT）建立音频/视频特征与语言嵌入之间的结构化对应关系，并将其作为伪标签指导对比学习以实现语义对齐。
result: 在LRS3-TED数据集上，该方法在纯净和各种信噪比的噪声环境下均优于强基线模型，取得了当前领先的性能。
conclusion: 通过将多模态特征锚定到LLM的语言空间，可以有效促进跨模态融合与解码，显著增强AVSR系统的鲁棒性。
---

## 摘要
基于大语言模型（LLM）的视听语音识别（LLM-AVSR）最近通过利用互补的音频和视觉信息，在恶劣的声学环境中表现出强大的鲁棒性。现有方法通常采用独立预训练的音频和视觉编码器，其输出被投影并融合为软提示（soft prompts），以引导 LLM 进行语音识别。然而，大多数方法在进行多模态融合时，没有明确解决音频、视觉和文本模态之间的表示差异，这可能限制了跨模态集成的有效性。在本文中，我们为 LLM-AVSR 提出了一种基于最优传输（OT）的语义对齐框架。该方法通过在多模态融合之前，参考 LLM 的语言嵌入空间对齐音频和视觉表示，从而明确地弥合了模态间隙。具体而言，OT 被用于估计概率耦合矩阵，这些矩阵刻画了特定模态特征与语言嵌入之间的结构化对应关系。生成的 OT 耦合进一步被用作软伪标签来监督对比学习，从而鼓励提取语义连贯且跨模态一致的视听表示。通过将多模态特征锚定到 LLM 的语言空间，所提出的框架促进了更有效的多模态融合和解码。我们使用基于 Whisper 的音频编码器、基于 AV-HuBERT 的视觉编码器和 LLaMA3.2-3B 解码器实现了所提出的框架。在 LRS3-TED 基准测试上进行的实验表明，该方法相比强基线模型有持续的改进，并在各种信噪比（SNR）下的清洁和噪声评估条件下均达到了最先进的性能。
