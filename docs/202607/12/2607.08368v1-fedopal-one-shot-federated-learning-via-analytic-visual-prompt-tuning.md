...
tags: ["query:vlm-da"]
score: 8.0
evidence: 在联邦学习中使用视觉提示微调来处理非独立同分布的数据分布。
tldr: 针对联邦学习中通信带宽受限和非独立同分布（Non-IID）数据导致的性能下降问题，本文提出FedOPAL框架。该框架结合了一次性联邦学习与解析学习，通过引入视觉提示作为特征矫正器，并施加局部近端约束，将异构数据特征映射至线性可分空间。FedOPAL在保持零服务器端训练成本的同时，显著提升了模型在异构环境下的准确率，为边缘大模型的高效协作提供了新方案。
source: arxiv
...
motivation: 传统解析联邦学习在处理非独立同分布数据时，因静态特征假设失效导致特征流形失真，限制了其在复杂边缘场景的应用。
method: 提出FedOPAL框架，利用视觉提示作为特征矫正器并结合局部近端约束，主动将异构数据分布修正为符合解析学习假设的线性可分空间。
result: 实验表明FedOPAL在多个基准数据集上显著优于原始解析方法，且在零服务器训练成本下达到了与最先进迭代方法相当的准确率。
conclusion: 本研究证明了通过视觉提示矫正特征分布可以有效解决解析联邦学习的局限性，为低功耗边缘智能协作提供了高效的工程范式。
---

## 摘要
随着基础模型在边缘智能中的广泛部署，通信带宽已成为限制联邦学习可扩展性的核心瓶颈。尽管单次联邦学习通过最小化通信轮数缓解了这一问题，但现有的迭代微调或知识蒸馏方法仍面临服务器端计算成本高和超参数敏感等挑战。解析联邦学习利用最小二乘闭式解实现了高效的无梯度聚合，但在非独立同分布（non-IID）数据环境下，其静态特征假设失效，导致特征流形失配，严重损害了模型性能。为解决这一矛盾，本文提出了 FedOPAL 框架。该框架将视觉提示作为特征矫正器，通过应用局部近端约束，主动将异构数据的特征分布修正到线性可分空间，从而满足解析联邦学习的理论假设。实验结果表明，FedOPAL 不仅在多个基准测试上显著优于原始解析方法，而且在保持服务器端零训练成本的同时，实现了与最先进迭代方法相当的准确率，为边缘大模型的高效协作提供了一种新的工程范式。
