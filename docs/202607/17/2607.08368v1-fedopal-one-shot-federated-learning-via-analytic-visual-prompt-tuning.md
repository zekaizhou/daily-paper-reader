...
tags: ["query:vlm-da"]
score: 7.0
evidence: 非独立同分布数据环境下联邦学习的视觉提示微调
tldr: 针对联邦学习中通信带宽受限和非独立同分布数据导致的性能下降问题，本文提出 FedOPAL 框架。该框架结合一轮联邦学习与解析学习，通过引入视觉提示作为特征矫正器，并施加局部近端约束，有效解决了异构数据下的特征流形失配问题。FedOPAL 在保持服务器端零训练成本的同时，实现了与最先进迭代方法相当的精度，为边缘大模型的高效协作提供了新方案。
source: arxiv
...
motivation: 现有一轮联邦学习方法在处理非独立同分布数据时，因解析学习的静态特征假设失效而导致特征流形失配，严重影响模型性能。
method: 提出 FedOPAL 框架，利用视觉提示作为特征矫正器，通过局部近端约束将异构数据特征分布修正至线性可分空间，以满足解析学习的理论假设。
result: FedOPAL 在多个基准测试上显著优于原始解析方法，并在零服务器训练成本下达到了与最先进迭代方法相当的准确率。
conclusion: 该研究证明了结合视觉提示与解析学习能有效应对联邦学习中的通信与异构性挑战，为边缘大模型的高效协作提供了新的工程范式。
---
